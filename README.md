...
- **Auth:** Auth.js v5 (`next-auth@beta`), email + password (bcrypt), JWT sessions, roles `guest` | `admin`. `middleware.js` guards `/account/*` (signed in) and `/admin/*` (admin). The **first account ever registered becomes admin**, as does any registration matching an address in `ADMIN_EMAIL` (one or many, comma-separated: `a@x.com,b@y.com`; the same list receives admin notifications unless `ENQUIRY_TO` is set). Admins can promote/demote from `/admin/guests/[id]` (the last admin can't be demoted).
