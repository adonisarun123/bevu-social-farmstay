...
export default function EnquiryForm() {
  const [data, setData] = useState({ name: "", phone: "", email: "", checkin: "", checkout: "", adults: "2", kids: "0", type: "room", message: "" });
  ...
  return (
    <form onSubmit={submit} className="card p-6 md:p-8">
      <input type="text" name="website" tabIndex={-1} autoComplete="off" className="hidden" aria-hidden="true" value={data.website || ""} onChange={set("website")} />
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2"><label className={label} htmlFor="email">Email <span className="normal-case tracking-normal text-stone">(optional)</span></label><input id="email" type="email" className={field} placeholder="you@example.com" value={data.email} onChange={set("email")} /></div>
      ...
      {status === "fallback" && <p className="mt-3 text-xs text-bark/70">We couldn't send that just now, so we opened WhatsApp with your enquiry pre-filled.</p>}
